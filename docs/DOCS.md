...
_NOTE 2: The NPSSO cookie is only needed to bootstrap an account. Once authenticated, the underlying PSN access/refresh tokens are persisted to disk (see [`PSN_AUTH_STORE_DIR`](#psn_auth_store_dir-optional) below), so a plain restart of the add-on (update, Home Assistant reboot, etc.) no longer requires re-entering it. That said, in testing the persisted refresh token's own validity appears to be a fixed ~10-day window anchored to the original NPSSO exchange — refreshing it (which happens automatically, roughly hourly while a console is awake, and on every add-on restart) rotates the token but does not appear to extend that window. So in practice, expect to provide a fresh NPSSO roughly every 10 days regardless of uptime; what this eliminates is having to do so on every restart within that window, not the periodic renewal itself. This isn't officially documented by Sony, so treat the ~10 days as an observed approximation rather than a guarantee. If both the persisted tokens and the configured NPSSO have expired, the add-on logs a clear message asking you to generate a new NPSSO instead of crashing — device discovery and control keep working regardless._
