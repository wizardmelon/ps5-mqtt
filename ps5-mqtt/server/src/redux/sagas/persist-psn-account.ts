...
// *whether* the tokens changed happens in-memory (see check-psn-presence.ts
// and the bootstrap in app.ts), so this saga only ever runs when there's
// genuinely something new to write — no per-tick disk reads to diff against.
